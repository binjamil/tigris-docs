...
For this quickstart we will create a database named _sampledb_ and then create a
collection _users_ inside it.

Creating a database is as easy as calling an API, and it is ready in an instant.

```shell
curl https://{your_tigris_uri}/v1/projects/quickstart/database/branches/sampledb/create \
      -X POST \
      -H "Authorization: Bearer your_token_here"
```

Similarly, a collection is instantaneously created through the API. In Tigris, a
collection is an ordered set of records called documents that all follow a
specific schema and whose fields are strongly typed.
